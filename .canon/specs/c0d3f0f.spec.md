...
title: "Canon GC"
domain: tools
created: 2026-02-23T14:12:17Z
depends_on: [b98ed16, e1bcd34, f3c2d71]
touched_domains: [canon-cli, ingest, ledger, state-render, tools]
---
# Canon GC
...
Canon is append only, so specs are immutable and fully auditable. Over time, a domain can accumulate many overlapping or superseded specs, making it difficult to read intent directly from `.canon/specs/`.
Introduce `canon gc` to consolidate a set of specs into fewer authoritative canonical specs while preserving history. Original specs stay discoverable through unchanged ledger entries and archived spec/source files.
## Core Behavior
1. `canon gc --domain <name>` consolidates all specs in a single domain.
2. `canon gc --specs <id1,id2,...>` consolidates a targeted set across domains.
3. Consolidation is AI driven and must produce a minimal set of active specs that keep the latest effective intent and remove redundant or stale content.
4. `canon gc` without `--write` runs as dry run and prints an execution plan including which specs would be archived, the proposed consolidated outputs, and reduced spec counts.
5. `canon gc --write` performs consolidation.
## Archive and Output Rules
1. All source specs in scope are archived, not deleted.
2. Archived spec files move from `.canon/specs/` to `.canon/archive/specs/` preserving original filenames.
3. Archived source snapshots move from `.canon/sources/` to `.canon/archive/sources/` preserving original filenames.
4. Ledger entries in `.canon/ledger/` are never modified or deleted.
5. New consolidated specs are written under `.canon/specs/` with fresh IDs.
6. Each new spec frontmatter includes a `consolidates` field listing replaced source spec IDs.
7. `canon init` must create `.canon/archive/specs/` and `.canon/archive/sources/`.
## Metadata and Lineage Preservation
1. `depends_on` for consolidated specs must preserve dependencies that point outside the consolidation set.
2. New ledger entries include normal parent links to the most recent head(s) at consolidation time.
3. Ledger entries must include a gc marker such as `gc: true` or `type: consolidation`.
## Safety and Control
1. `canon gc` must validate semantic consistency using `canon check` logic before consolidation.
2. If unresolved conflicts are found in the target set, gc must abort and report them.
3. `--force` can override the minimum count threshold only, never conflict safety.
4. `--min-specs <n>` defaults to `5`.
`canon gc [options]`
- `--root <path>` repository root default `.`
- `--domain <name>`
- `--specs <id1,id2,...>`
...
- `--response-file <path>`
## AI Requirements
1. Use the configured AI provider from `.canonconfig` unless overridden.
2. AI prompt must preserve active non superseded requirements, resolve contradictions by `created` recency, and remove redundancy.
3. Output should be structured markdown specs with clear sections and retained domain/type metadata.
1. Unit tests for archive moves, preserved ledger history, `consolidates` frontmatter, dependency preservation, conflict gate, and `--min-specs` behavior.
2. E2E dry run test with no filesystem side effects.
3. E2E write test with archival and creation of new specs.
4. E2E tests showing `canon log` still reports full history and `canon render --write` remains consistent after gc.
