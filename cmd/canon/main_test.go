...
func TestSemanticDiffCommandRejectsIncompleteGitRefs(t *testing.T) {
	err := run([]string{"semantic-diff", "--base-ref", "main"})
	if err == nil {
		t.Fatalf("expected missing head-ref error")
	}
	if !strings.Contains(err.Error(), "--base-ref and --head-ref must be provided together") {
		t.Fatalf("unexpected git refs validation error: %v", err)
	}
}

func TestSemanticDiffCommandRejectsDiffFileWithGitRefs(t *testing.T) {
	root := t.TempDir()
	diffPath := writeCLISemanticDiffFixtureDiff(t, root, `diff --git a/a.txt b/a.txt
index 1111111..2222222 100644
--- a/a.txt
...
`)
	...
	if err == nil {
		t.Fatalf("expected diff-file and refs validation error")
	}
	if !strings.Contains(err.Error(), "--diff-file cannot be used with --base-ref/--head-ref") {
		t.Fatalf("unexpected diff-file refs validation error: %v", err)
	}
}
