...
type goTestJSONTerminalParseResult struct {
	Outcomes                      map[testFlakinessKey]testFlakinessOutcome
	FailedPackagesWithoutOutcomes []string
}
...
func runGoTestJSONTerminalActions(root string, packages []string) (map[testFlakinessKey]testFlakinessOutcome, error) {
	...
	parseResult, parseErr := parseGoTestJSONTerminalActions(output)
	if parseErr != nil {
		if err != nil {
	...
	if len(parseResult.FailedPackagesWithoutOutcomes) > 0 {
		pkgList := strings.Join(parseResult.FailedPackagesWithoutOutcomes, ", ")
		if err != nil {
			return nil, fmt.Errorf("go test -json failed for package(s) without test-level outcomes: %s: %w\n%s", pkgList, err, strings.TrimSpace(string(output)))
		}
		return nil, fmt.Errorf("go test -json failed for package(s) without test-level outcomes: %s", pkgList)
	}

	if len(parseResult.Outcomes) == 0 {
		if err != nil {
	...
	return parseResult.Outcomes, nil
}

func parseGoTestJSONTerminalActions(raw []byte) (goTestJSONTerminalParseResult, error) {
	scanner := bufio.NewScanner(bytes.NewReader(raw))
	scanner.Buffer(make([]byte, 0, 64*1024), 8*1024*1024)

	terminalActions := make(map[testFlakinessKey]testFlakinessOutcome)
	packageHasTerminalOutcome := make(map[string]bool)
	...
	for scanner.Scan() {
		...
		if testName != "" {
			switch testFlakinessOutcome(action) {
			case testFlakinessOutcomePass, testFlakinessOutcomeFail, testFlakinessOutcomeSkip:
				terminalActions[testFlakinessKey{Package: pkg, Test: testName}] = testFlakinessOutcome(action)
				packageHasTerminalOutcome[pkg] = true
			}
			continue
		}

		if action == "fail" {
			failedPackages[pkg] = struct{}{}
		}
	}
	if err := scanner.Err(); err != nil {
		return goTestJSONTerminalParseResult{}, err
	}
	if !sawJSONEvent {
		return goTestJSONTerminalParseResult{}, errors.New("go test -json returned no parsable JSON events")
	}

	failedWithoutOutcomes := make([]string, 0)
	for pkg := range failedPackages {
		if packageHasTerminalOutcome[pkg] {
			continue
		}
		failedWithoutOutcomes = append(failedWithoutOutcomes, pkg)
	}
	sort.Strings(failedWithoutOutcomes)

	return goTestJSONTerminalParseResult{
		Outcomes:                      terminalActions,
		FailedPackagesWithoutOutcomes: failedWithoutOutcomes,
	}, nil
}
