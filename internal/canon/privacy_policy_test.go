...
func TestResolvePrivacyCheckScopePathsRejectsEscapesAndMissingPaths(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(outsideRoot, "outside.go"), []byte("package outside\n"), 0o644); err != nil {
		t.Fatalf("failed writing outside.go: %v", err)
	}
	...
	if _, err := resolvePrivacyCheckScopePaths(root, []string{"outside-link/outside.go"}); err == nil || !strings.Contains(err.Error(), "via symlink") {
...
func TestNormalizePrivacyEvidencePathsDropsWindowsAbsolutePaths(t *testing.T) {
	root := t.TempDir()

	paths := normalizePrivacyEvidencePaths([]string{
		"src/inside.go",
		`C:\Users\alice\outside.go`,
		`C:/Users/alice/outside.go`,
		`\\server\share\outside.go`,
		`\Windows\outside.go`,
	}, root)

	if len(paths) != 1 || paths[0] != "src/inside.go" {
		t.Fatalf("expected only in-repo relative path, got: %+v", paths)
	}
}
