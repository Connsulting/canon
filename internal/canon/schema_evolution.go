...
func stripSchemaEvolutionSQLComments(sql string) string {
	...
	for i := 0; i < len(src); i++ {
		...
		if inDollarQuote {
			if schemaEvolutionMatchesDelimiter(src, i, dollarDelimiter) {
		...
		if c == '/' && i+1 < len(src) && src[i+1] == '*' {
			...
			dst[i] = ' '
			for i+1 < len(src) {
				if src[i+1] == '*' && i+2 < len(src) && src[i+2] == '/' {
					i++
					dst[i] = ' '
					i++
					dst[i] = ' '
...
func schemaEvolutionMatchesDelimiter(src []byte, index int, delimiter []byte) bool {
	if len(delimiter) == 0 || index < 0 {
		return false
	}
	...
	return bytes.Equal(src[index:end], delimiter)
}
