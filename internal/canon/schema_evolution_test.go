...
func TestSchemaEvolutionDoesNotSplitSemicolonsInsideDollarQuotedLiterals(t *testing.T) {
	root := t.TempDir()
	writeSchemaEvolutionMigrationFile(t, root, "db/migrations/001_dollar_semicolon.sql", `ALTER TABLE users ADD COLUMN note TEXT DEFAULT $body$drop table users; alter table users drop column id;$body$;`)
	...
	if result.StatementCount != 1 {
		t.Fatalf("expected 1 statement, got %d", result.StatementCount)
	}
	if len(result.Findings) != 0 {
		t.Fatalf("expected no findings, got %d: %+v", len(result.Findings), result.Findings)
	}
