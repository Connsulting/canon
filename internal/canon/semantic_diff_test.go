...
func TestSemanticDiffValidationAndErrorPaths(t *testing.T) {
	...
	if _, err := SemanticDiff(root, SemanticDiffOptions{AIMode: "from-response", ResponseFile: responsePath, BaseRef: "main"}); err == nil {
		t.Fatalf("expected missing head-ref error")
	} else if !strings.Contains(err.Error(), "both --base-ref and --head-ref are required") {
		t.Fatalf("unexpected missing head-ref error: %v", err)
	}

	refRoot := t.TempDir()
	runSemanticGit(t, refRoot, "init")
	runSemanticGit(t, refRoot, "config", "user.email", "semantic-diff@example.com")
	runSemanticGit(t, refRoot, "config", "user.name", "Semantic Diff")
	if err := os.WriteFile(filepath.Join(refRoot, "a.txt"), []byte("before\n"), 0o644); err != nil {
		t.Fatalf("failed writing ref fixture: %v", err)
	}
	runSemanticGit(t, refRoot, "add", "a.txt")
	runSemanticGit(t, refRoot, "commit", "-m", "seed")

	refResponsePath := writeSemanticDiffResponseFixture(t, refRoot, "semantic-response.json", map[string]any{
		"model":        "codex-headless",
		"summary":      "summary",
		"explanations": []map[string]any{},
	})
	if _, err := SemanticDiff(refRoot, SemanticDiffOptions{
		AIMode:       "from-response",
		ResponseFile: refResponsePath,
		BaseRef:      "missing-base-ref",
		HeadRef:      "HEAD",
	}); err == nil {
		t.Fatalf("expected unknown ref error")
	} else if !strings.Contains(err.Error(), "git diff failed for refs") {
		t.Fatalf("unexpected unknown ref error: %v", err)
	}
}
