...
func SemanticDiff(root string, opts SemanticDiffOptions) (SemanticDiffResult, error) {
	...
	input, err := collectSemanticDiffInput(absRoot, strings.TrimSpace(opts.DiffFile), strings.TrimSpace(opts.BaseRef), strings.TrimSpace(opts.HeadRef))
	if err != nil {
		return SemanticDiffResult{}, err
	}
...
func collectSemanticDiffInput(root string, diffFile string, baseRef string, headRef string) (SemanticDiffInput, error) {
	diffSource := "git"
	diffText := ""
	var err error
	...
	if diffFile != "" {
		if baseRef != "" || headRef != "" {
			return SemanticDiffInput{}, fmt.Errorf("--diff-file cannot be used with --base-ref/--head-ref")
		}
		...
		diffText, diffSource, err = readSemanticGitDiff(root, baseRef, headRef)
		if err != nil {
...
func readSemanticGitDiff(root string, baseRef string, headRef string) (string, string, error) {
	source := "git"
	refRange := ""
	args := []string{"diff", "--no-ext-diff", "--unified=0"}
	if baseRef != "" || headRef != "" {
		if baseRef == "" || headRef == "" {
			return "", "", fmt.Errorf("both --base-ref and --head-ref are required when using git refs")
		}
		refRange = fmt.Sprintf("%s..%s", baseRef, headRef)
		source = "git:" + refRange
	...
	if err != nil {
		if source != "git" {
			return "", "", fmt.Errorf("git diff failed for refs %s: %w\n%s", refRange, err, strings.TrimSpace(string(output)))
		}
		return "", "", fmt.Errorf("git diff failed: %w\n%s", err, strings.TrimSpace(string(output)))
	}
	return string(output), source, nil
}
