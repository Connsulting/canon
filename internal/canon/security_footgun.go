...
func collectSecurityFootgunGoFiles(root string) ([]string, error) {
	...
	err := filepath.WalkDir(root, func(pathAbs string, entry fs.DirEntry, walkErr error) error {
		...
		rel := toRelativeSlash(root, pathAbs)
		if entry.IsDir() {
...
func securityFootgunDetectCompositeLit(
	...
	for _, elt := range lit.Elts {
		...
		if securityFootgunExprNameEquals(kv.Key, "InsecureSkipVerify") {
			if v, ok := securityFootgunBoolLiteral(kv.Value); ok && v {
...
func securityFootgunDetectExecShell(
	...
	shell, ok := securityFootgunStringLiteral(call.Args[argStart])
	if !ok {
		return
	}
	flag, ok := securityFootgunStringLiteral(call.Args[argStart+1])
	if !ok {
		return
	}

	shellNormalized := strings.ToLower(strings.TrimSpace(shell))
	flagNormalized := strings.ToLower(strings.TrimSpace(flag))
	if _, ok := securityFootgunShellNames[shellNormalized]; !ok {
		return
	}
	...
	appendFinding(SecurityFootgunFinding{
		...
		Column:   column,
		Snippet:  snippet,
		Message:  fmt.Sprintf("os/exec %s invokes shell %q with %q", sel.Sel.Name, shell, flag),
	})
}
...
func securityFootgunDetectInsecureRand(
	...
	if imports[pkgIdent.Name] != "math/rand" {
		return
	}
	...
	appendFinding(SecurityFootgunFinding{
		...
		Column:   column,
		Snippet:  snippet,
		Message:  fmt.Sprintf("math/rand used in security-sensitive context (%s); prefer crypto/rand", reason),
	})
}
